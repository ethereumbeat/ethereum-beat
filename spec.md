...
## 33. Pass 22 — social embeds: dynamic OG card + Farcaster Mini App

Make the site render a strong card when its URL is pasted into a feed, and a
launchable Farcaster Mini App on the primary (BEAT) channel. Zero paid
dependencies. Non-financial throughout; nothing here touches metric_meta, needs
no D1 migration or KV bust, and does not mutate the committed wrangler.toml.

A. DYNAMIC OG IMAGE `/og/beat.png`. An Astro endpoint (`prerender = false`) that
   renders the current BEAT state to a 1200×800 PNG (3:2, the ratio Farcaster
   requires). It reads ONLY `snapshot:latest` from the `SNAP` KV binding — never
   the RPC or D1 — and composes the card from three sources: the first featured
   metric's value/label (KV snapshot), the live slot number (pure clock maths,
   `src/lib/clock.ts`), and the CROPS taxonomy line CR · O · P · S. Rendering is
   satori (HTML/JSX → SVG) + @resvg/resvg-wasm (SVG → PNG) — both run on
   ...
   the repo. Fonts: satori needs sfnt (ttf/otf), and the repo ships only woff2, so
   a build script (`scripts/build-og-assets.mjs`, using the MIT `wawoff2` decoder,
   a devDependency) decompresses the existing Departure Mono woff2 to OTF and
   base64-inlines it into a committed module — no new font binaries fetched, no
   paid deps. The pre-baked fallback PNG is generated by the same script and
   committed base64-inline, so it is available even if the render path throws.

B. SITE-WIDE SOCIAL META. The shared head already emits og:title, og:description,
   og:type, twitter:card=summary_large_image and twitter:image; this pass points
   og:image / twitter:image at the absolute `/og/beat.png` and corrects the
   declared height to 800 for that 3:2 card. The description already leads
   non-financial (Ethereum network health; no price, no market) and is unchanged.

C. FARCASTER MINI APP EMBED. A `fc:miniapp` meta tag (NOT the deprecated
   `fc:frame`) carries stringified JSON: version "1", imageUrl → `/og/beat.png`
   (PNG, 3:2), a button title ≤ 32 chars, action.type "launch_miniapp", url → the
   site root, splashImageUrl → `/splash.png`, and splashBackgroundColor `#fbfbf9`
   from the palette. Emitted site-wide (it is one inert meta tag and always points
   at root), so any pasted channel URL still offers the Mini App launch.

D. `/.well-known/farcaster.json`. An endpoint returning the manifest `frame`
   block: version "1", name, iconUrl → `/icon.png`, homeUrl → root,
   splashImageUrl, splashBackgroundColor. The `accountAssociation`
   (header/payload/signature that binds the domain to a Farcaster FID) is READ
   FROM the `FARCASTER_ACCOUNT_ASSOCIATION` env var / secret and parsed in; it is
   omitted when the var is absent. It is NEVER fabricated — the maintainer signs
   the domain with their custody key and injects the value as a secret. Until
   then the manifest serves without accountAssociation (valid, but the app is
   unverified). This is the one manual step this pass cannot automate.

E. ICON + SPLASH ASSETS. `public/icon.png` (1024×1024, opaque — Farcaster app
   icons must have no alpha) and `public/splash.png` (200×200) are rasterised from
   the existing glyph (`public/favicon.svg` — ring + Ethereum diamond + one red
   tick) by the icon build script, monochrome + red on paper. Wired into C and D.

F. CSP. The Farcaster web client renders a Mini App inside an iframe, which the
   site's `frame-ancestors 'none'` + `X-Frame-Options: DENY` would block. This
   pass relaxes framing to a tight allowlist — `frame-ancestors 'self'` plus the
   Farcaster/Warpcast client hosts — and drops the blanket `X-Frame-Options: DENY`
   (that header cannot express a multi-host allowlist; modern browsers honour the
   CSP directive instead). No other directive changes: `img-src 'self' data:`
   already covers the same-origin OG image; the PNG and JSON endpoints are
   non-HTML so they carry no CSP. `scripts/audit-csp.mjs` is updated in lockstep
   to assert the new frame-ancestors allowlist (and continues to reject any
   regression to unsafe script-src).

Gates: clean `npm run build`; audit-meta green and extended to assert, on the
homepage, that og:image is present + resolvable, twitter:card is present, and
