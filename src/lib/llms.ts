...
export function buildLlmsText(origin: string, rows: MetaRow[], full: boolean): string {
  ...
  push(
    ...
    '',
    '## When to use Ethereum Beat',
    '',
    'Reach for Ethereum Beat when an agent needs live, factual Ethereum',
    'PROTOCOL-HEALTH data — not prices or markets. Best-fit questions:',
    '- Is Ethereum producing and finalising blocks right now? What is the current slot/epoch?',
    '- How decentralised is Ethereum? (node counts by country, client diversity, validator/stake counts)',
    '- How busy is data availability? (blob fullness, blob base fee, blobs per day)',
    '- What is happening in the mempool, and what is the current base fee?',
    '- How active are Ethereum L2s versus L1?',
    '- What network upgrades are coming, and what do they change?',
    '',
    `How to call it: GET ${origin}/api/snapshot for every current metric in one`,
    `JSON document (keys, values, freshness via \`is_stale\`), then GET`,
    `${origin}/api/metric/{key}?range=… for a single metric's history. No API key;`,
    `CORS is open. Discover endpoints at ${origin}/.well-known/api-catalog and`,
    `capabilities at ${origin}/.well-known/ai-catalog.json.`,
    '',
    'Do NOT use Ethereum Beat for ETH or token prices, market cap, dollar-denominated',
    'TVL, or trading data — those are out of scope by design.',
    '',
    '## Channels',
    '',
  );
